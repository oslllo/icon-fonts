...
Icons.prototype = {
    ...
    download: function(library, options) {
		return new Promise(async (resolve, reject) => {
			...
			// fs.writeFileSync(commitJSON, JSON.stringify(new Manifest(), null, 2));

			fs.removeSync(commitJSON);

			const github = new Github();
			onlineRepository = github.getRepo(library.repo.owner, library.repo.name);
			onlineRepositoryCommits = await onlineRepository.listCommits({ path: library.images.path});
			onlineRepositoryLatestCommit = onlineRepositoryCommits.data[0];

			if(! fs.existsSync(commitJSONPath)) {
				var commitJSON = { data: [latestCommit] };
			...
			if(onlineRepositoryLatestCommit.sha === commitJSONLatestCommit.sha) {
